function SwotCard({ titulo, Icon, items, isExpandido, isBloqueado, onClick, gradient }) {
  ...
  return (
    <div 
      ...
      } ${isBloqueado ? 'relative cursor-not-allowed' : 'cursor-pointer hover:shadow-xl'}`}
    >
      {isBloqueado && (
      ...
      {isExpandido && items.length > 0 ? (
        ...
        <p className="text-white/80 text-sm sm:text-base">Clique para expandir</p>
      )}
    </div>
  );
}
